...
namespace VarinaCmsV2.Security.Contracts
{
    public interface IAppUserManager:IDisposable
    {
      ...
      //  Task<bool> IsCurrentUserInRoleAsync(string role);
        ...
        void SeedDatabase(IUserManagerSeedDataBaseContext context);
        /// <summary>
        /// Used to hash/verify passwords
        /// </summary>
        IPasswordHasher PasswordHasher { get; set; }

        IEnumerable<Premission> GetUserPremissions(User user);
       ...
       IEnumerable<Role> GetRoles(User user);
        ...
        User FindById(Guid userId,bool includeRoles=false);
        User GetCurrentUser();

        bool HasCurrentUserAccsess( string action);

        Task<User> FindByPhoneNumberAsUserNameAsync(string userName, string password);
        bool ExistsByUrl(string userUrl);
        Task<User> FindByUrlAsync(string userUrl);
        List<User> GetAdminstrators();
    }
}