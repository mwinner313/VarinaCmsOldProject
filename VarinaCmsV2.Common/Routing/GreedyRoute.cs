...
namespace VarinaCmsV2.Common.Routing
{
    ...
    public class GreedyRoute : Route
    {
        ...
        {
            Stack<string> parts = new Stack<string>(virtualPath.Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries).Reverse());
            ...
            while (!finished)
            {
                if (currentSegment.Value.IsToken && !currentSegment.Value.IsGreedy)
                {
                ...
                if (currentSegment.Value.IsToken && currentSegment.Value.IsGreedy && currentSegment.Next == null)
                {
                    var str = string.Empty;
                    do
                    {
                        str = str  + (p ?? this.Defaults[currentSegment.Value.Name]) + "/";
                     

                        p = parts.Pop();
                        ;
                    } while (p != null);
                    result.Add(currentSegment.Value.Name, str.Trim('/'));
                    finished = true;
                    continue;
                }
                ...
                if (currentSegment.Value.IsToken && currentSegment.Value.IsGreedy && currentSegment.Next != null)
                {
                    ...
                    do
                    {
                        str = str  + (p ?? this.Defaults[currentSegment.Value.Name]) + "/";
                        p = parts.Pop();
                    } while (!currentSegment.Next.Value.Name.Equals(p));
                    result.Add(currentSegment.Value.Name, str.Trim('/'));
                    currentSegment = currentSegment.Next;
                    continue;
                }
